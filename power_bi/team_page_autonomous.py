...
def main():
    data = data_manipulation.source_to_list(f"{pycharm_path}/2020Data.xlsx")
    ...
    autonomos_data = list(filter(lambda x: x['matchPart'] == 'autonomos', team_data))
    points = [team_page.balls_by_game(autonomos_data, x) for x in range(1, 4)]
    games = []
